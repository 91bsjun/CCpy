...
from CCpy.Gaussian.myGausssum.gausssum.utils import GaussianSpectrum, levelname, percent
from CCpy.Gaussian.myGausssum.gausssum.cclib.parser.utils import convertor

# from Tkinter import *
import matplotlib as mpl
#mpl.use('Agg')
from matplotlib import pyplot as plt
from .mympl import MPLPlot
from .plot import DisplayPlot
import CCpy.Gaussian.myGausssum.gausssum.myfolder as myfolder
#import mpld3
...
##    os.sep = "/"
    ...
    print("Creating the EDDM script file for %s\n" % fchkpoint)
...
def ET(logfile,logfilename,start,end,numpts,FWHM):

    #print("Starting to analyse the electronic transitions\n")
...
# Read in orbital_data.txt if it exists(which contains info on the contribs)
    ...
    orbdata = False
    try:
        inputfile=open(os.path.join("./GaussSum_tmp/orbital_data.txt"),"r")
    except IOError:
        pass
    else:
        ...
        orbdata=True
        #print("Using orbital_data.txt\n")
        if thisHOMO != list(logfile.homos):
            #print("Disagreement on HOMO...orbital_data.txt says "+str(thisHOMO)+"\n"+logfile.filename+" says "+str(logfile.homos)+"\n")
            return False
        screenCD=[]
        for i in range(len(logfile.etenergies)): # For each transition
            screenCD.append("")
...
# Write UVData.txt containing info on the transitions
    ...
    #print("Writing the transition info to UVData.txt\n")
    outputfile=open(os.path.join(gaussdir,"UVData.txt"),"w") ### output directory !!!!
    #outputfile=open("/var/www/FlaskApp/FlaskApp/static/GaussResults/UVData.txt","w") ### output directory !!!!
    ...
    t = GaussianSpectrum(startwaveno,endwaveno,numpts,
                         ...
                         FWHM)
    #print("Writing the spectrum to UVSpectrum.txt\n")
    outputfile=open(os.path.join(gaussdir,"UVSpectrum.txt"),"w")
    #outputfile=open("/var/www/FlaskApp/FlaskApp/static/GaussResults/UVSpectrum.txt","w")
    ...
    if max(t.spectrum[0,:])<1E-8: # Don't draw it if the spectrum is flat
        print("There are no peaks in this wavelength range!\n")
    else:
        g = MPLPlot()
        xvalues_nm = [convertor(x,"cm-1","nm") for x in t.xvalues]
        g.setlabels("Wavelength (nm)", "epsilon")
        ...
        g.subplot2.set_xlim(left=start, right=end)
        g.secondaxis.set_ylim(bottom=0)
        mpl = g.makechart()
        return mpl
        #DisplayPlot(root, g, "UV-Vis Spectrum") ############plot here!!!!!!!!!
...
#######################################################
    ...
    if not hasattr(logfile,"etrotats"):
        print("No rotatory lengths were found\n")
        return False
    elif len(logfile.etrotats)!=len(logfile.etenergies): # Catch violation of an assertion
        print("The number of R values doesn't agree with the number of electronic transitions!\n")
        return False
    ...
    print("Writing the spectrum to CDSpectrum.txt\n")
    ...
    if max(t.spectrum[0,:])<1E-8: # Gnuplot won't draw it if the spectrum is flat
        ...
        g2 = MPLPlot2()
        g2.setlabels("Wavelength (nm)", r"$\epsilon$")

        xvalues_nm = [convertor(x,"cm-1","nm") for x in t.xvalues]
        g2.data(zip(xvalues_nm,t.spectrum[0,:]))
        energies_nm = [convertor(x,"cm-1","nm") for x in logfile.etenergies]
        oscdata = [(x, y) for x, y in zip(energies_nm, logfile.etrotats) if start < x < end]
        g2.data(oscdata, vlines=True, y2axis="R (length) / $10^{-40}$")

        g2.subplot.set_xlim(left=start, right=end)
        g2.secondaxis.set_ylim(bottom=0)
        mpl3 = g2.makechart()
    return mpl,mpl3
'''
#######################################################
