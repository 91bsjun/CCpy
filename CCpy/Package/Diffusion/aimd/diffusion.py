...
class ArreheniusAnalyzer(object):
    ...
    def get_custom_arrhenius_plot(self, color, label, marker, temp, show_room_temp, rng_diffusivity):
        ...
        plt.scatter(self.x, self.diffusivities, marker=marker, s=100, linewidth=2, facecolors='w', edgecolors=color, label=label)
        ...
        (_, caps, _) = plt.errorbar(self.x, self.diffusivities, yerr=self.diffusivity_errors, fmt=marker, mfc='w', color=color, capsize=5)
        for cap in caps:
        ...
        (_, caps, _) = plt.errorbar([1000./temp], [d_ext], yerr=[[d_error_lower], [d_error_upper]], fmt=marker, mfc='w', color=color, capsize=5)
        for cap in caps:
            cap.set_markeredgewidth(2)
